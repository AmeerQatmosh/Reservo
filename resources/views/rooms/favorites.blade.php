...
@section('content')
    ...
    <div class="mt-10 min-w-0">
        @if ($rooms->count() === 0)
            ...
            >
                <div class="rounded-2xl bg-yellow-50 p-4 text-yellow-600 ring-1 ring-yellow-200/70">
                    <x-lucide name="star" class="h-8 w-8" aria-hidden="true" />
                </div>
                <p class="mt-4 text-sm font-semibold text-gray-900">{{ __('No favourites yet') }}</p>
                <p class="mt-1 max-w-md text-xs text-gray-600">
                    {{ __('Use the star on any room card to save it here for quick access.') }}
                </p>
                <a
                    href="{{ route('rooms.index') }}"
                    class="mt-5 inline-flex items-center gap-2 rounded-xl bg-teal-600 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-teal-700"
                >
                    {{ __('Browse rooms') }}
