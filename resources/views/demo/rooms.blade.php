...
@section('content')
    @php
        ...
        $demoRoomsViewQuery = $browseView === 'list' ? ['view' => 'list'] : [];
        $demoRoomsResetUrl = route('demo.rooms', $demoRoomsViewQuery);
        $demoViewToggleBase = request()->query();
        $demoGridUrl = route('demo.rooms', array_merge($demoViewToggleBase, ['view' => 'grid', 'page' => 1]));
        $demoListUrl = route('demo.rooms', array_merge($demoViewToggleBase, ['view' => 'list', 'page' => 1]));
    @endphp
    <x-page-breadcrumbs
        class="mb-4"
        :items="[
    ...
    <div
        class="relative mt-0 flex w-full min-w-0 flex-col gap-4 lg:grid lg:grid-cols-12 lg:items-start lg:gap-6"
    >
        <input
            type="checkbox"
            id="demo-room-browse-filters-toggle"
            ...
            aria-controls="demo-room-browse-filters-aside"
        />
        <label
            for="demo-room-browse-filters-toggle"
        ...
        ></label>
        <aside
            id="demo-room-browse-filters-aside"
            ...
            aria-labelledby="demo-room-browse-filters-title"
        >
            <div
            ...
            >
                ...
                >
                    <h2
                        id="demo-room-browse-filters-title"
        ...
        <div
            class="flex min-w-0 w-full flex-col gap-4 lg:col-span-9 lg:col-start-4 lg:row-start-1"
        >
            <div
                class="flex w-full min-w-0 flex-wrap items-center gap-2 sm:gap-3 lg:flex-nowrap lg:justify-between lg:pb-1"
            >
                    <div class="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
                        <div class="min-w-0 flex-1">
                            @include('rooms.partials.room-filters-search', [
                                'filters' => $filters,
                                'filterFormId' => 'demo-rooms-browse-filters',
                            ])
                        </div>
                        <label
                            for="demo-room-browse-filters-toggle"
                        ...
                        <div class="flex min-w-0 flex-col gap-3">
                            @foreach ($rooms as $room)
                                @include('rooms.partials.room-browse-item', [
                                    'room' => $room,
                                    'layout' => 'list',
                                    'itemUrl' => route('demo.room.show', $room['id']),
                                    'hourlyLabel' => \App\Support\DemoState::hourlyRateLabel(isset($room['hourly_rate']) ? (float) $room['hourly_rate'] : null),
