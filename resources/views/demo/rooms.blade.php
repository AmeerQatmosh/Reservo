...
@section('content')
    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
            ...
            <p class="mt-2 text-sm text-gray-600">
                Same filters as production: text search, capacity, size, location, amenities, and sort.
            </p>
        </div>
    ...
    <div class="mt-6 rounded-2xl border border-white/70 bg-white/90 p-4 shadow-sm sm:p-5">
        <form method="GET" action="{{ route('demo.rooms') }}" class="space-y-4">
    ...
    @else
        <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @foreach ($rooms as $room)
                ...
                @endphp
                <a href="{{ route('demo.room.show', $room['id']) }}" class="overflow-hidden rounded-3xl border border-white/70 bg-white/90 shadow-sm transition hover:-translate-y-0.5 hover:border-gray-200 hover:shadow-md">
                    <div class="aspect-[16/10] overflow-hidden bg-gray-100">
                        @if (! empty($room['image_url']))
                            <img src="{{ $room['image_url'] }}" alt="{{ $room['name'] }}" class="h-full w-full object-cover" loading="lazy">
                        @else
                            <div class="flex h-full w-full items-center justify-center bg-gradient-to-br from-slate-100 to-indigo-50 text-xs text-gray-500">
                                No photo
                            </div>
                        @endif
                    </div>
                        ...
                        @if (count($amenities))
                            <div class="mt-3 flex flex-wrap gap-1.5">
                                ...
                                @if (count($amenities) > 4)
                                    <span class="rounded-md bg-slate-50 px-2 py-0.5 text-[11px] text-slate-500">+{{ count($amenities) - 4 }} more</span>
                                @endif
                            </div>
                        @endif
                        <div class="mt-4 line-clamp-3 text-sm leading-6 text-gray-700">
                            {{ \Illuminate\Support\Str::limit($room['description'] ?? '', 220) }}
                        </div>
                    </div>
                </a>
            @endforeach
