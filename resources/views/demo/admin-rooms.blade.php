...
@section('content')
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="{{ route('demo.hub') }}" class="text-sm font-medium text-gray-700 hover:text-gray-900">Demo home</a>
        </div>
    </div>

    <div class="mt-6 rounded-2xl border border-white/70 bg-white/90 p-4 shadow-sm sm:p-5">
        <form method="GET" action="{{ route('demo.admin.rooms') }}" class="space-y-4">
            ...
            <div class="border-t border-gray-200 pt-4">
                ...
                <div class="mt-3 flex flex-wrap items-end gap-x-2 gap-y-3 sm:gap-x-3 lg:flex-nowrap lg:gap-x-3">
                    <div class="w-[6.75rem] shrink-0 sm:w-[7.25rem]">
                        ...
                        <input id="min_capacity" name="min_capacity" type="number" min="1" value="{{ $filters['min_capacity'] ?? '' }}" class="app-field">
                    </div>
                    <div class="w-[6.75rem] shrink-0 sm:w-[7.25rem]">
                        <label for="max_capacity" class="block text-sm font-medium text-gray-900">Max. capacity</label>
                        <input id="max_capacity" name="max_capacity" type="number" min="1" value="{{ $filters['max_capacity'] ?? '' }}" class="app-field">
                    </div>
                    <div class="w-[6.75rem] shrink-0 sm:w-[7.25rem]">
                        <label for="min_size_sqm" class="block text-sm font-medium text-gray-900">Min. size (m²)</label>
                        <input id="min_size_sqm" name="min_size_sqm" type="number" min="1" value="{{ $filters['min_size_sqm'] ?? '' }}" class="app-field">
                    </div>
                    <div class="w-[6.75rem] shrink-0 sm:w-[7.25rem]">
                        <label for="max_size_sqm" class="block text-sm font-medium text-gray-900">Max. size (m²)</label>
                        <input id="max_size_sqm" name="max_size_sqm" type="number" min="1" value="{{ $filters['max_size_sqm'] ?? '' }}" class="app-field">
                    </div>
                ...
                <label class="mt-4 flex cursor-pointer items-center gap-2 text-sm text-gray-800">
                    <input type="checkbox" name="has_photo" value="1" class="rounded border-gray-300 text-gray-900 focus:ring-gray-900" @checked(! empty($filters['has_photo']))>
                        ...
                        @foreach ($rooms as $room)
                            <tr>
                                <td class="px-4 py-3 align-middle">
                                    <a href="{{ route('demo.admin.rooms.show', $room['id']) }}" class="block h-14 w-[4.5rem] overflow-hidden rounded-lg bg-gray-100 ring-1 ring-gray-200/80">
                                ...
                                <td class="px-4 py-3 font-medium text-gray-900">
                                    <a href="{{ route('demo.admin.rooms.show', $room['id']) }}" class="hover:underline">{{ $room['name'] }}</a>
                                </td>
                                <td class="px-4 py-3 text-gray-600">{{ $room['location'] ?: '—' }}</td>
                                <td class="px-4 py-3">{{ $room['capacity'] }}</td>
                                <td class="px-4 py-3 text-gray-700">{{ \App\Support\DemoState::hourlyRateLabel(isset($room['hourly_rate']) ? (float) $room['hourly_rate'] : null) ?? '—' }}</td>
                                <td class="px-4 py-3">
                                    <div class="flex flex-wrap justify-end gap-2">
                                        <a href="{{ route('demo.admin.rooms.show', $room['id']) }}" class="app-table-action">View</a>
                                        <form method="POST" action="{{ route('demo.admin.rooms.destroy', $room['id']) }}" class="inline-flex" data-confirm-message="Remove this sandbox room and its bookings?" data-confirm-variant="danger" data-confirm-button-label="Remove">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="app-table-action app-table-action-danger">Delete</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
