...
@section('content')
    <div class="mx-auto max-w-3xl">
        <h1 class="text-3xl font-semibold tracking-tight text-gray-900 sm:text-4xl">Try the full booking experience</h1>
        ...
        <div class="mt-10 grid gap-4 sm:grid-cols-3">
            <form method="POST" action="{{ route('demo.start') }}" class="flex flex-col rounded-3xl border border-white/70 bg-white/90 p-6 shadow-sm transition hover:border-gray-200 hover:shadow-md">
                ...
                <button type="submit" class="mt-6 w-full rounded-2xl bg-teal-600 px-4 py-3 text-sm font-medium text-white transition hover:bg-teal-700">Start as user</button>
            </form>
            <form method="POST" action="{{ route('demo.start') }}" class="flex flex-col rounded-3xl border border-white/70 bg-white/90 p-6 shadow-sm transition hover:border-gray-200 hover:shadow-md">
                @csrf
                ...
                <button type="submit" class="mt-6 w-full rounded-2xl bg-teal-600 px-4 py-3 text-sm font-medium text-white transition hover:bg-teal-700">Start as admin</button>
            </form>
            <form method="POST" action="{{ route('demo.start') }}" class="flex flex-col rounded-3xl border border-white/70 bg-white/90 p-6 shadow-sm transition hover:border-gray-200 hover:shadow-md">
                @csrf
                ...
                <button type="submit" class="mt-6 w-full rounded-2xl bg-teal-600 px-4 py-3 text-sm font-medium text-white transition hover:bg-teal-700">Start as super admin</button>
            </form>
        </div>
    </div>
@endsection
